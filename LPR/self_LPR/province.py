...
'''def get_pro(a): # and sys.argv[1]=='predict':
    ...
    with tf.Session() as sess:
        ...
        for h in range(a.shape[0]):
            for w in range(a.shape[1]):
                img_data[0][w+h*width] = a[h][w]/255
    ...
    return [PROVINCES[max1_index], max1*100]
    '''
def get_pro(a):
    ...
    with tf.Session(graph=g1) as sess:
        ...
        #a = cv2.imread('test_images/1.bmp')
        #a = cv2.cvtColor(a, cv2.COLOR_RGB2GRAY)
        for i in range(1,7):
            a = cv2.imread('pro_imgs/2 (%s).bmp'%i)
            a = cv2.cvtColor(a, cv2.COLOR_RGB2GRAY)
            width = a.shape[1]
            img_data = [[0]*SIZE for i in range(1)]
            for h in range(a.shape[0]):
